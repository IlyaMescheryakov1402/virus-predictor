...
class FeatureExtractionTransformer():
    def __init__(self):
      self.only_viruses = set()
      self.only_clear = set()


    def fit(self, X, y):
      res = list(chain.from_iterable(list(map(lambda x: x.split(','), list(X[y == 1])))))
      res = self.clearing(res)
      res_nv = list(chain.from_iterable(list(map(lambda x: x.split(','), list(X[y == 0])))))
      res_nv = self.clearing(res_nv)
      self.only_viruses = set(res) - set(res_nv)
      self.only_clear = set(res_nv) - set(res)
      return self
    

    def clearing(self, data):
      data = list(map(lambda x: re.sub(r'\.(.+)$', r'', x).strip(), data))
      data = list(map(lambda x: re.sub(r'c:\\\\\\\\\\\\/\\/\\/\\/\\//\\\\\\\\\\\\\\\\\\\\\\\\\\\\//////\\\\/\\/\\/windows\\\\\\\\\\\\/\\/\\/\\/\\//\\\\\\\\\\\\\\\\\\\\\\\\\\\\//////\\\\/\\/\\/system32\\\\\\\\\\\\/\\/\\/\\/\\//\\\\\\\\\\\\\\\\\\\\\\\\\\\\//////\\\\\\/\\/\\/\\/\\\\/\\\\/\\/\\/\\/\\/\\\\/\\/\\/\\/\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\/\\/\\/', r'', x), data))
      return data
...
clff=CatBoostClassifier(eval_metric='F1', random_state=42, metric_period=20, n_estimators=2000, max_depth=5, silent=True)
clff.fit(X_trans, y)

with open('model.pkl','wb') as f:
    pickle.dump(clff,f)

with open('only_viruses.pkl','wb') as f:
    pickle.dump(prep.named_transformers_['feature_ext'].named_steps['Feature_extraction'].only_viruses,f)

with open('only_clear.pkl','wb') as f:
    pickle.dump(prep.named_transformers_['feature_ext'].named_steps['Feature_extraction'].only_clear,f)